...
- Vai trò khác: chỉ trả nhóm lãnh đạo (Manager + Admin) — đủ để chọn người chat, không lộ danh sách đồng nghiệp
...
```json
{
  "mon_001": { "ten_mon": "Cà phê đen", "gia": 25000, "loai": "cafe", "con_hang": true },
  "mon_002": { "ten_mon": "Trà sữa trân châu", "gia": 45000, "loai": "tra", "con_hang": true }
}
```
...
Xóa món. *(Yêu cầu Manager/Admin)* — ID món mới luôn sinh theo `max + 1` nên **không** cần đánh lại số thứ tự sau khi xóa.
...
> Nhận **một trong hai** header: `Authorization: Bearer <token>` (client) hoặc `X-Server-Secret` (ChatServer gọi nội bộ).

### GET `/chat/messages?roomId=<roomId>`

Lấy lịch sử tin nhắn của một phòng.

**Query:** `roomId` — ID phòng chat (vd: `room_global` cho phòng chung, `chat_nv_001_nv_002` cho chat riêng 2 người)

**Response 200:**

```json
[
  { "nguoi_gui_id": "nv_001", "ten_nguoi_gui": "Nguyễn Văn A", "noi_dung": "Hello!", "loai_tin_nhan": "text", "thoi_gian": 1746536400000 },
  { "nguoi_gui_id": "nv_002", "ten_nguoi_gui": "Trần Thị B", "noi_dung": "Hi!", "loai_tin_nhan": "text", "thoi_gian": 1746536410000 }
]
```
...
Lưu tin nhắn vào database (bình thường do **ChatServer** gọi sau khi broadcast — client không gọi trực tiếp).
Server chuẩn hóa field và tự điền `thoi_gian` nếu thiếu.
